...
static void* sysCreateAnonShmem(size_t length)
{
    ...
    if (ptr == MAP_FAILED) {
        QUP_LOGI("mmap(%d, RW, SHARED|ANON) failed: %s", (int) length,
            strerror(errno));
        return NULL;
    }
    ...
    return ptr;
#else
    QUP_LOGI("sysCreateAnonShmem not implemented.");
    return NULL;
#endif
}
...
static int getFileStartAndLength(int fd, off_t *start_, size_t *length_)
{
    ...
    if (start == (off_t) -1 || end == (off_t) -1) {
        QUP_LOGI("could not determine length of file");
        return -1;
    }

    length = end - start;
    if (length == 0) {
        QUP_LOGI("file is empty");
        return -1;
    }
...
int sysFakeMapFile(int fd, MemMapping* pMap)
{
    ...
    */
    ...
    if (read(fd, memPtr, length) < 0) {
        QUP_LOGI("read(fd=%d, start=%d, length=%d) failed: %s", (int) length,
...
int sysMapFileInShmem(int fd, MemMapping* pMap)
{
    ...
    memPtr = mmap(NULL, length, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_FILE | MAP_PRIVATE,
            fd, start);
    if (memPtr == MAP_FAILED) {
        QUP_LOGI("mmap(%d, R/W, FILE|PRIVATE, %d, %d) failed: %s", (int) length,
            fd, (int) start, strerror(errno));
        return -1;
    }
...
int sysMapFileInShmemWritableReadOnly(int fd, MemMapping* pMap)
{
    ...
    if (mprotect(memPtr, length, PROT_READ) < 0) {
        ...
        int err = errno;
        QUP_LOGI("mprotect(%p, %d, PROT_READ) failed: %s",
            memPtr, length, strerror(err));
        QUP_LOGI("mprotect(RO) failed (%d), file will remain read-write", err);
    }
    ...
    MemMapping* pMap)
{
    ...
    memPtr = mmap(NULL, actualLength, PROT_READ, MAP_FILE | MAP_SHARED,
                fd, actualStart);
    if (memPtr == MAP_FAILED) {
        QUP_LOGI("mmap(%d, R, FILE|SHARED, %d, %d) failed: %s",
    ...
    QUP_LOGI("mmap seg (st=%d ln=%d): bp=%p bl=%d ad=%p ln=%d",
    ...
    return 0;
#else
    QUP_LOGI("sysMapFileSegmentInShmem not implemented.");
    return -1;
#endif
}
    ...
    MemMapping* pMap)
{
    ...
    {
        QUP_LOGI("Attempted to change %p; map is %p - %p",
...
void sysReleaseShmem(MemMapping* pMap)
{
    ...
    if (munmap(pMap->baseAddr, pMap->baseLength) < 0) {
        QUP_LOGI("munmap(%p, %zd) failed: %s",
            pMap->baseAddr, pMap->baseLength, strerror(errno));
    } else {
        QUP_LOGI("munmap(%p, %zd) succeeded", pMap->baseAddr, pMap->baseLength);
        pMap->baseAddr = NULL;
        pMap->baseLength = 0;
    }
#else
...
{
    while (count != 0) {
        int actual = write(fd, buf, count);
        if (actual < 0) {
            int err = errno;
            QUP_LOGI("%s: write failed: %s", logMsg, strerror(err));
            return err;
        } else if (actual != (ssize_t) count) {
            QUP_LOGI("%s: partial write (will retry): (%d of %zd)",
                logMsg, (int) actual, count);
			// buf = dynamic_cast<const void*>(dynamic_cast<const u1*>(buf) + actual);
...
int sysCopyFileToFile(int outFd, int inFd, size_t count)
{
    ...
    while (count != 0) {
        ...
        ssize_t actual = read(inFd, buf, getSize);
        if (actual != (ssize_t) getSize) {
            QUP_LOGI("sysCopyFileToFile: copy read failed (%d vs %zd)",
                (int) actual, getSize);
            return -1;
        }
